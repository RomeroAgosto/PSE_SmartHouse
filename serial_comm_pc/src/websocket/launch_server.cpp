...
void launch_websocket_server(unsigned int port, boost::mutex *mutex1) {
  ...
  interface_api.on_message = [](std::shared_ptr<WsServer::Connection> connection, std::shared_ptr<WsServer::Message> message) {
  ...
  interface_api.on_open = [](std::shared_ptr<WsServer::Connection> connection) {
    cout << "Server (websocket): Opened connection " << connection.get() << endl;
		ALLCONNECTIONS=websocket_server.get_connections();
  };

  interface_api.on_close = [](std::shared_ptr<WsServer::Connection> connection, int status, const string & /*reason*/) {
    cout << "Server (websocket): Closed connection " << connection.get() << " with status code " << status << endl;
  };

  interface_api.on_error = [](std::shared_ptr<WsServer::Connection> connection, const SimpleWeb::error_code &ec) {
