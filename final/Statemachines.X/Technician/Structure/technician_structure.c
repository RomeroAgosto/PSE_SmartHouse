...
int set_new_thresholds(char *message,int p) {
    ...
    for (i = 0; i < 2; i++) {
        for(j = 0;j < 5; j++) {
            printf("air_quality_hysteresis %d\n\r ",(message[message_counter] - '0') * 10 + message[message_counter+1] - '0');
            set_air_quality_hysteresis(i,j,(message[message_counter] - '0') * 10 + message[message_counter+1] - '0'); /*hysteresis air quality*/
            //printf("message %d,message counter %d\n",message[message_counter]-'0',message_counter);
            //printf("message %d,message counter %d\n",message[message_counter+1]-'0',message_counter+1);
    ...
    for (j = 0; j < 2; j++) {
         for(k=0;k<5;k++) {
             printf("threshold %d\n ",(message[ message_counter] - '0') * 1000 + (message[message_counter+1] - '0') * 100 + (message[message_counter +2 ] - '0')*10 + message[message_counter +3 ] - '0');
             set_air_quality_threshold(j,k,(message[ message_counter] - '0') * 1000 + (message[message_counter+1] - '0') * 100 + (message[message_counter +2 ] - '0')*10 + message[message_counter +3 ] - '0');

         }
    }

    for(i=0;i<8;i++) {
        printf("air_temp_hysteresis %d\n\r ",message[message_counter]-'0');
        set_air_temp_hysteresis(i,message[message_counter]-'0');
    }
    printf("water_temp_hysteresis %d\n\r ",message[message_counter]-'0');
    set_water_hysteresis(message[message_counter]-'0');
    for (i = 0;i  < 4; i++) {
        printf("light cycle %d\n\r ",(message[message_counter]-'0')*10+message[message_counter+1]-'0');
        set_light_cycles(i,(message[message_counter]-'0')*10+(message[message_counter+1]-'0'));
    }
}
int set_light_cycles(int light_number, int cycle_time){
    message_counter=message_counter+2;
...
int set_air_quality_threshold(int risk_level, int threshold, int value){
    ...
    message_counter=message_counter+4;
}
